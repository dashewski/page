...
export default function MobileCaseHero({ t, lang, setLang, color }: MobileCaseHeroProps) {
  ...
  return (
    <section className="flex flex-col pt-[20px] w-full">
      ...
      {/* Breadcrumb nav */}
      <div className="flex flex-col gap-[15px] mb-[30px]" style={{ fontFamily: VISITOR }}>
        <div className="flex items-center gap-[15px]">
          <span className="text-[#aeaeae] text-[30px] leading-none cursor-pointer hover:opacity-70" onClick={() => router.push('/')}>
            {t.hero.breadcrumb}
          </span>

        </div>
        <div className="flex items-center gap-[15px]">
          <span className="text-[#aeaeae] text-[30px] leading-none">|#cases</span>
                   <span className="text-[#2c2c2c] text-[15px] cursor-pointer hover:text-[#aeaeae] transition-colors" style={{ fontFamily: CHAKRA }} onClick={() => { window.location.href = '/#mobileExamples' }}>
            {t.hero.back}
          </span>
        </div>
        <div className="flex items-center gap-[15px]">
          <span className="text-[30px] leading-none" style={{ color }}>{t.hero.tag}</span>
          <span className="text-[#2c2c2c] text-[15px]" style={{ fontFamily: CHAKRA }}>← YOU HERE</span>
        </div>
      </div>

      {/* Name */}
      <p className="text-[#aeaeae] text-[70px] leading-none mb-[20px]" style={{ fontFamily: VISITOR }}>
        {t.hero.name}
      </p>
